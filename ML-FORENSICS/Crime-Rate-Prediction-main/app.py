...
# Load the forensic prediction model
...
def predict_result(): 
    # City Mapping (19 Metropolitan Cities)
    city_names = { 
    ...
    # Base Population (2011 Census in Lakhs)
    population = { 
    ...
    # Demographic Growth Projection (1% annual CAGR since 2011)
    year_diff = int(year) - 2011
    pop_adjusted = base_pop + (0.01 * year_diff * base_pop)

    # Perform Regression Analysis
    crime_rate = model.predict([[year, city_code, pop_adjusted, crime_code]])[0] 
    
    city_name = city_names[city_code] 
    crime_type = crimes_names[crime_code] 
    
    # Forensic Risk Categorization
    if crime_rate <= 1:
    ...
    # Calculate estimated incident count
    cases = math.ceil(crime_rate * pop_adjusted)
